#ifndef _square_root_h
#define _square_root_h

#include <basic_math_operations.h>
#include <stdlib.h>
#include <string.h>

char *square_root(const char *number_in, size_t accuracy) {
  size_t shift = 0;
  ...
  for (size_t i = 0; i < number_len - 1; i += 2) {
    ...
    while (start <= end) {
      ...
      char *_temp_ =
          (char *)calloc(strlen(working_space) + strlen(multiplication) + 3, 1);
