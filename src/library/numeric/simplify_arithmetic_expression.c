...
static void
str_replace_all(char **str_in, const char *from, const char *to)
{
  char *str = *str_in;
...
static void
remove_misplaced_and_redundant_signs(char **expression_in)
{
...
static size_t
get_corresponding_closing_bracket(const char *str, size_t index)
{
...
static void
replace_substring_from_position(size_t startPosition, size_t endPosition,
                                char **string_in, const char *replacement)
{
  char *string = *string_in;

  size_t replacement_len = strlen(replacement);

  char *answer = (char *)calloc(strlen(string) - (endPosition - startPosition + 1) + replacement_len + 1,
                                1);
...
static long
find_operational_sign(const char *expression, char sign)
{
...
static size_t
get_back_corresponding_square_bracket(const char *str, size_t index)
{
  if (str[index] != ']')
    return -1;
  int count = 0;
...
static bool
equal_to_any_from(const char *options, char n, size_t size)
{
...
static char *
get_numerical_arguments(const char *expression, bool fromLeft,
                        long *signIndexIn, int outputType)
{
  ...
  if (fromLeft)
  {
    ...
    {
      size_t back_corresponding_square_bracket = get_back_corresponding_square_bracket(expression, signIndex);
    ...
    {
      // No brackets
      while (signIndex >= 0 && !equal_to_any_from(operators, expression[signIndex], numberOfOperators) && !encounteredMinusSign)
      {
        if (expression[signIndex] == '-')
        {
          // Check if this minus sign is an operator or a sign
          bool is_not_operator = signIndex == 0 || expression[signIndex - 1] == '(' || expression[signIndex - 1] == '[' || expression[signIndex - 1] == '{' || expression[signIndex - 1] == '^' || expression[signIndex - 1] == '*' || expression[signIndex - 1] == '/' || expression[signIndex - 1] == '+' || expression[signIndex - 1] == '-';
          if (!is_not_operator)
          {
  ...
  else
  {
    ...
    {
      size_t corresponding_closing_bracket = get_corresponding_closing_bracket(expression, signIndex);
      start = signIndex + 1;
    ...
    {
      start = signIndex;
      while (signIndex < (long long)strlen(expression) && !equal_to_any_from(operators, expression[signIndex], numberOfOperators) && !(expression[signIndex] == '-' && encounteredNumber))
      {
...
get_chain_division_location(char *expression, long *sign1In, long *sign2In)
{
  ...
  while (divisionSignLocation >= 0)
  {
    ...
    expression[divisionSignLocation] = '^'; // This is just temporary, it's so we don't pull division
                                            // signs in the right argument.
    char *rightArgument = get_numerical_arguments(expression, false, &rightArgumentEnd, 1);
    expression[divisionSignLocation] = '/';
    if (rightArgumentEnd + 1 < (long long)strlen(expression) && expression[rightArgumentEnd + 1] == '/')
    {
      free(rightArgument);
                               ...
                               size_t accuracy)
{
  ...
  while (_loc != NULL)
  {
    ...
    char *simplifiedInnerExpression = simplify_arithmetic_expression(
        innerExpression, outputType, accuracy);
    ...
    {
      struct fraction innerExpressionFraction = parse_fraction(simplifiedInnerExpression);
      // Reciprocate the fraction.
      if (innerExpressionFraction.numerator[0] == '-')
      {
        char *tempSwap = (char *)calloc(
            strlen(innerExpressionFraction.numerator), 1);
        strcpy(tempSwap, innerExpressionFraction.numerator + 1);
        innerExpressionFraction.numerator = (char *)realloc(
      ...
      {
        char *tempSwap = (char *)calloc(
            strlen(innerExpressionFraction.numerator) + 1, 1);
    ...
    bool haveToChangeBrackets = (end + 1 < strlen(expression) && expression[end + 1] == '^') || (start >= 1 && expression[start - 1] == '^');
  ...
  while (signLocation >= 0)
  {
    ...
    char *leftArgument = get_numerical_arguments(expression, true, &start, outputType);
    char *rightArgument = get_numerical_arguments(expression, false, &end, outputType);
    if (outputType == 0)
    ...
    if (outputType == 1)
    {
      ...
      strcpy(simplifiedExponentiation + squareBrackets, answer.numerator);
      simplifiedExponentiation[strlen(answer.numerator) + squareBrackets] = '/';
      strcpy(simplifiedExponentiation + strlen(answer.numerator) + 1 + squareBrackets,
             answer.denominator);
      if (squareBrackets)
  ...
  if (outputType == 0)
  {
    ...
    {
      long start = signLocation, end = signLocation;
      char *numerator = get_numerical_arguments(expression, true, &start, outputType);
      char *denominator = get_numerical_arguments(expression, false, &end, outputType);
  ...
  else
  {
    ...
    while (sign1 >= 0 && sign2 >= 0)
    {
      ...
      long div1End = sign1;
      long div2End = sign2;
      char *numerator = get_numerical_arguments(
          expression, true, &numeratorStart, outputType);
      // Again, this is just temporary (to avoid pulling division signs)
      expression[sign1] = '^';
      char *div1 = get_numerical_arguments(expression, false, &div1End,
                                           outputType);
      expression[sign1] = '/';
      expression[sign2] = '^'; // Avoid pulling division signs
      char *div2 = get_numerical_arguments(expression, false, &div2End,
                                           outputType);
      expression[sign2] = '/';
      char *denominator = (char *)calloc(strlen(div1) + strlen(div2) + 3, 1);
      multiply(div1, div2, denominator);
      char *simplifiedChainDivision = (char *)calloc(
          strlen(numerator) + strlen(denominator) + 2, 1);
      strcpy(simplifiedChainDivision, numerator);
      simplifiedChainDivision[strlen(numerator)] = '/';
      strcpy(simplifiedChainDivision + strlen(numerator) + 1,
             denominator);
      replace_substring_from_position(
          numeratorStart, div2End, &expression, simplifiedChainDivision);
      get_chain_division_location(expression, &sign1, &sign2);
      free(numerator);
      free(div1);
      free(div2);
    ...
    while (signLocation >= 0)
    {
      ...
      if (outputType == 0)
      {
        operationResult = (char *)calloc(
            strlen(leftArgument) + strlen(rightArgument) + 3, 1);
        switch (sign)
        {
        case '*':
      ...
      else
      {
        ...
        operationResult = (char *)calloc(
            strlen(answer.numerator) + strlen(answer.denominator) + 2,
            1);
        strcpy(operationResult, answer.numerator);
        operationResult[strlen(answer.numerator)] = '/';
        strcpy(operationResult + strlen(answer.numerator) + 1,
               answer.denominator);
  ...
  char *denominatorBiggerCheck = (char *)calloc(
  ...
  size_t expression_buf_len = strlen(quotient) + strlen(remainder) + strlen(frac.denominator) + 6;

  expression = (char *)realloc(expression, expression_buf_len);
  size_t charactersWritten = 0;
  if (sign == '-')
    expression[charactersWritten++] = '-';
  strcpy(expression + charactersWritten, quotient);
  ...
  if (strcmp(remainder, "0"))
  {
    ...
    expression[charactersWritten++] = ' ';
    strcpy(expression + charactersWritten, remainder);
    charactersWritten += strlen(remainder);
    expression[charactersWritten++] = '/';
    strcpy(expression + charactersWritten, frac.denominator);
