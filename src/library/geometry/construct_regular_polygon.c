  ...
  for (size_t i = 0; i < n - 2; ++i)
    {
      ...
      char *buf_1 = (char *)calloc(strlen(polygon[i].y) + strlen(polygon[i + 1].y) + 3, 1);
      char *buf_2 = (char *)calloc(strlen(polygon[i].x) + strlen(polygon[i + 1].x) + 3, 1);
      subtract(polygon[i].y, polygon[i + 1].y, buf_1);
      subtract(polygon[i].x, polygon[i + 1].x, buf_2);
      char *slope_initial_line = (char*)calloc(strlen(buf_1)+strlen(buf_2)+accuracy+3,1);
      divide(buf_1,buf_2,slope_initial_line,accuracy);

      // Compute the slope and intercept of the new line.
      char *buf_3 = (char *)calloc(strlen(slope_initial_line)+strlen(tan_ext_angle)+3,1);
      char *buf_4 = (char *)calloc(strlen(slope_initial_line)+strlen(tan_ext_angle)+3,1);
      ...
      char *slope_new_line = (char *)calloc(strlen(buf_3)+strlen(buf_5)+accuracy+3,1);
      divide(buf_3,buf_5,slope_new_line,accuracy);
      char *buf_6 = (char*)calloc(strlen(slope_new_line)+strlen(polygon[i + 1].x)+3,1);
      multiply(slope_new_line,polygon[i + 1].x,buf_6);
      char *intercept_new_line=(char*)calloc(strlen(polygon[i + 1].y)+strlen(buf_6)+3,1);
      subtract(polygon[i + 1].y,buf_6,intercept_new_line);

      // Compute candidate intersection points.
      char *buf_7 = (char *)calloc (2*strlen(intercept_new_line)+3,1);
      multiply(intercept_new_line,intercept_new_line,buf_7);
      char *buf_8 = (char *)calloc(strlen(buf_7)+4,1);
      add(buf_7,"1",buf_8);
      char *buf_9 = square_root(buf_8, accuracy);
      char *buf_10 = (char *)calloc(strlen(length)+strlen(buf_9)+accuracy+3,1);
      divide(length,buf_9,buf_10,accuracy);
      char *x_1 = (char *)calloc(strlen(polygon[i + 1].x)+strlen(buf_10)+3,1);
      char *x_2 = (char *)calloc(strlen(polygon[i + 1].x)+strlen(buf_10)+3,1);
      add(polygon[i + 1].x, buf_10, x_1);
      subtract(polygon[i + 1].x, buf_10, x_2);
      char *buf_11 = (char *)calloc(strlen(x_1)+strlen(slope_new_line)+3,1);
      char *buf_12 = (char *)calloc(strlen(x_2)+strlen(slope_new_line)+3,1);
      multiply(slope_new_line,x_1,buf_11);
      multiply(slope_new_line,x_2,buf_12);
      char *y_1 = (char *)calloc(strlen(buf_11)+strlen(intercept_new_line)+3,1);
      char *y_2 = (char *)calloc(strlen(buf_12)+strlen(intercept_new_line)+3,1);
      ...
      if (i+1>n/2){
        char *sub_buf = (char *)calloc(strlen(polygon[i + 1].y)+strlen(y_1)+3,1);
      ...
      if (!strcmp(y_1,y_2)) {
        char *sub_buf = (char *)calloc(strlen(x_1)+strlen(x_2)+3,1);
