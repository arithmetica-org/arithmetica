...
char *arcsin(const char *number, size_t accuracy) {
  accuracy++;
  char one[] = "1";
  char two[] = "2";
  ...
  while (1) {
    ...
    char *__buf = (char *)calloc(strlen(two_n_factorial) + strlen(buf) + 1, 1);
    multiply(two_n_factorial, buf,
             __buf); // __buf = two_n_factorial * i * (i + 1)
    two_n_factorial = (char *)realloc(two_n_factorial, strlen(two_n_factorial) +
                                                           strlen(buf) + 1);
    ...
    n_factorial =
        (char *)realloc(n_factorial, strlen(n_factorial) + strlen(j) + 1);
    ...
    char *_den_1 =
        (char *)calloc(strlen(two_power_n) + strlen(n_factorial) + 1, 1);
    char *den_1 =
        (char *)calloc(2 * (strlen(two_power_n) + strlen(n_factorial)) + 1, 1);
